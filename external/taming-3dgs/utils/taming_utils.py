...
def compute_gaussian_score(scene, camlist, edge_losses, gaussians, pipe, bg, importance_values, opt, to_prune=False):
    ...
    for view in range(len(camlist)):
        ...
        gaussian_importance[visibility_filter] += agg_importance[visibility_filter]
...
def get_count_array(start_count, multiplier, opt, mode, num_steps=None):
    # Eq. (2) of taming-3dgs
    if mode == "multiplier":
    ...
    if num_steps is None:
        num_steps = sum(
            1
            for iteration in range(opt.densify_from_iter + 1, opt.densify_until_iter)
    ...
    values = [int(1*a * (x**2) + (b * x) + c) for x in range(num_steps + 1)]
