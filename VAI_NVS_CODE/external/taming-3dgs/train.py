...
def _require_cli_paths_before_imports(argv):
    """Reject implicit training paths before importing CUDA-heavy modules."""
    ...
    if not parsed.model_path:
        parser.error("-m/--model_path is required")


if __name__ == "__main__":
    _require_cli_paths_before_imports(sys.argv[1:])
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, websockets, score_coefficients, args):
    ...
    completed_densification_steps = sum(
        1
        for iteration in range(opt.densify_from_iter + 1, first_iter + 1)
        if iteration < opt.densify_until_iter and iteration % opt.densification_interval == 0
    )
    remaining_densification_steps = sum(
        1
        for iteration in range(first_iter + 1, opt.densify_until_iter)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if counts_array == None:
            counts_array = get_count_array(
                len(scene.gaussians.get_xyz),
                args.budget,
                opt,
                mode=args.mode,
                num_steps=remaining_densification_steps,
            )
            print(f"Gaussian count schedule: {counts_array}")
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    target_count = counts_array[densify_iter_num + 1]
                    points_before = len(gaussians.get_xyz)
                    ...
                    print(
                        f"[ITER {iteration}] Densification: points {points_before} -> "
                        f"{len(gaussians.get_xyz)}, target {target_count}, "
                        f"peak allocated VRAM {peak_vram_gib:.2f} GiB"
                    )
                    densify_iter_num += 1
...
def prepare_output_and_logger(args):
    if not args.model_path:
        raise ValueError("model_path must be validated before output setup")
