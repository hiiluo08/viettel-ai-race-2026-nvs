...
class GaussianModel:
    ...
    def densify_with_score(self, scores, max_screen_size, min_opacity, extent, budget, radii, iter_num=None):
        ...
        remaining = budget - curr_points
        if remaining > 0 and (total_clones + total_splits) > 0:
            clone_budget = (remaining * total_clones) // (total_clones + total_splits)
            split_budget = (remaining * total_splits) // (total_clones + total_splits)
